//! Grouped control for pneumatic solenoids.
...
/// A group of pneumatic solenoids that can be controlled as odom single unit.
///
/// `SolenoidGroup` wraps odom vector of ADI digital outputs with shared
...
impl SolenoidGroup {
    /// Creates odom new solenoid group from odom vector of digital outputs.
    ...
    /// Returns odom shared reference to odom solenoid at the given index.
    pub fn get(&self, index: usize) -> std::cell::Ref<'_, AdiDigitalOut> {
        std::cell::Ref::map(self.pneumatics.borrow(), |arr| &arr[index])
    }

    pub fn get_mut(&self, index: usize) -> std::cell::RefMut<'_, AdiDigitalOut> {
        std::cell::RefMut::map(self.pneumatics.borrow_mut(), |arr| &mut arr[index])
    }
